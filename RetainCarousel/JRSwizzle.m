...
+ (BOOL)jr_copyMethodImplemention:(SEL)origSel_ toMethod:(SEL)altSel_ error:(NSError **)error_ {
#if OBJC_API_VERSION >= 2
	Method origMethod = class_getInstanceMethod(self, origSel_);
	if (!origMethod) {
#if TARGET_OS_IPHONE
    ...
    method_setImplementation(class_getInstanceMethod(self, altSel_), method_getImplementation(class_getInstanceMethod(self, origSel_)));
	return YES;
#else
	//	Scan for non-inherited methods.
	Method directOriginalMethod = NULL, directAlternateMethod = NULL;
	...
	//	Swizzle.
	directAlternateMethod->method_imp = directOriginalMethod->method_imp;
	
	return YES;
#endif
}

+ (BOOL)jr_copyClassMethodImplemention:(SEL)origSel_ toMethod:(SEL)altSel_ error:(NSError **)error_ {
    return [GetClass((id)self) jr_copyMethodImplemention:origSel_ toMethod:altSel_ error:error_];
}
